...
def get_platform_profiles(platform):
    ...
    url = ('%s/%s_ts.txt') % (seatrec_root_url, platform)
    logging.warning('get_platform_profiles(%s)' % url)
    try:
...
def set_color_lims(vehicle, sensor, df):
    ...
    """
    logging.warning('set_color_lims(%s, %s)' % (vehicle, sensor))
...
def make_seatrec_fig(platform, sensor, sensor_df):
    ...
    """
    logging.warning("make_seatrec_fig(%s)" % platform)
...
def plotly_seatrec_3D(platform, sensor, sensor_df):
    ...
    Notes: Working on migrating settings into config file
    """
    logging.warning('plotly_seatrec_3D(%s, %s)' % (platform, sensor))
    ...
    logging.warning("plotly_seatrec_3D(): plotting %s" % sensor)
    logging.warning("plotly_seatrec_3D(): start_date %s" % min_date)
    logging.warning("plotly_seatrec_3D(): end_date %s" % max_date)
...
def get_last_seatrec_profile(platform, platform_df):
    ...
    """
    logging.warning("get_last_seatrec_profile(%s)", platform)
    last_profile_n = platform_df['Profile Number'].max()
    logging.warning("get_last_seatrec_profile(%s): Last Profile Number is %d",
...
def seatrec_plot_sensor(platform, platform_df, sensor):
    ...
    logging.warning('seatrec_plot_sensor(%s, %s)' % (platform, sensor))
    ...
    logging.warning("seatrec_plot_sensor(%s): plotting %s", platform, sensor)
    logging.warning("seatrec_plot_sensor(%s): start_date %s", platform, min_date,)
    logging.warning("seatrec_plot_sensor(%s): end_date %s", platform, max_date)
...
def add_seatrec_logo(vehicle,fig):
    ...
    """
    logging.warning('add_logo(%s)' % vehicle)
...
def seatrec_plot_last_profile(platform, last_profile, sensor):
    ...
    logging.warning('seatrec_plot_last_profile(%s): Plotting %s', platform, sensor)
    # Begin plot config
    title_fs = 10
    suptitle_fs = 12
    title_style = 'italic'
...
def seatrec_surface_marker(platform, platform_df):
    ...
    """
    logging.warning('seatrec_surface_marker(%s): creating surface marker' % platform)
    coords = []
    ...
    surf_marker.properties['html'] = """
        <h5><center><span class='warningBoxHeading'>Seatrec Float Status</span></center></h5>
        <table class='warningBoxTable'>
            <tr><td class='td_warningBoxSensor'>Platform:</td><td>%s</td></tr>
            <tr><td class='td_warningBoxSensor'>PI:</td><td>%s</td></tr>
            <tr><td class='td_warningBoxSensor'>Date/Time:</td><td>%s</td></tr>
            <tr><td class='td_warningBoxSensor'>Position:</td><td>%0.4fW/%0.4fN</td></tr>
        </table>
        <div class = 'warningBoxBreakBar'>Most Recent Cycle</div>
        <div class = 'warningBoxLastPlotDiv'>
            <a href='%s' target="_blank">
                <img class = 'warningBoxLastPlotImage' src = '%s'></img>
            </a>
            <a href='%s' target="_blank">
                <img class = 'warningBoxLastPlotImage' src = '%s'></img>
            </a>
        </div>
        <div class = 'warningBoxBreakBar'>2D Time Series Plots</div>
        <div class = 'warningBoxPlotDiv'>
            <a href='%s' target="_blank">
                <img class = 'warningBoxPlotImage' src = '%s'></img>
            </a>
            <a href='%s' target="_blank">
                <img class = 'warningBoxPlotImage' src = '%s'></img>
            </a>
...
def profiles_to_df(profiles):
    ...
    """
    logging.warning('profiles_to_df()...')
    # Get column names
    headers = profiles[0]
...
def gen_seatrec_profiles(platform, platform_df):
    ...
    """
    logging.warning('gen_seatrec_profiles(%s)', platform)
    maxp = int(platform_df['Profile Number'].max())
    logging.warning('gen_seatrec_profiles(%s): %d profiles', platform, maxp)
    idx = 1
    while idx <= maxp:
...
def create_profile(platform, seatrec_xr, profile):
    ...
    """
    logging.warning('create_profile(%s): %d' % (platform, profile))
    ...
    for argo_var in seatrec_config['ARGO_variables']:
        ...
        for record in seatrec_config['ARGO_variables'][argo_var]['var_keys']:
            ...
            if isinstance(value, str):
                #logging.warning('STRING', argo_var, record, value)
                command = "temp_name.%s = '%s'" % (record, value)
                logging.debug("%s", command)
            else:
                #logging.warning('NOT STRING', argo_var, record, value)
...
def get_seatrec_config(platform):
    ...
    Created:    2022-06-22
    Modified:   2023-03-31
    Notes:      Gets all seatrec config warning for creating nc files. Vars, etc.
...
def process_seatrec_data(platform_list):
    ...
    for platform in platform_list:
        ...
        if platform_profiles:
            ...
            logging.warning("platform_df has %d rows" % len(platform_df))
...
def get_platform_ids():
    ...
    logging.warning(f'get_platform_ids(): Got {platforms}')
    return (platforms)
...
def seatrec_process():
    ...
    if len(platforms) > 0:
        ...
        logging.warning(f'seatrek_process(): No platforms fetched...')
