...
def create_profile(dataset, profile):
    ...
    Notes:      profile is a special case and can't easily be constructed
                ...
                to handle the few gdac vars like this...
    """
    logging.debug('create_profile(%d)' % profile)
...
def drop_dims(vehicle, sg_ds):
    ...
    """
    config = get_sg_config(vehicle)
    if config['config_settings']['drop_sg_data']:
        try:
            ...
            logging.warning("drop_dims(%s): No sg_data_point dim.",
                            vehicle)
            return False
    if (config['config_settings']['drop_legato_data']):
        try:
