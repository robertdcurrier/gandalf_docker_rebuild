...
def gen_last_pos(vehicle, df):
    ...
    """
    logging.info('gen_last_pos(%s): max ctd_time %d',vehicle,
...
def chunk_it(vehicle):
    ...
    """
    chunks.append(df)
    print(df['ctd_time'].max())
    sys.exit()

    # Now we iterate
    while chunk_end < numdocs:
        chunk_start = chunk_start+chunk_size
        chunk_end = chunk_end+chunk_size
        logging.info('chunk_it(%s) start: %d end: %d',vehicle,
                     chunk_start, chunk_end)
        results = list(db.sgdata.find(myquery)[chunk_start:chunk_end])
        df = pd.DataFrame(results)
        chunks.append(df)
    
    catdog = pd.concat(chunks)
    catdog = catdog.sort_values(by = ['ctd_time'], ascending=[True])
    return(catdog)
    """
    return df
...
def gen_df(vehicle):
    ...
    df_len = (len(df))
    if df_len == 0:
        logging.debug('gandalf_sg_track(): Empty Data Frame')
        return
