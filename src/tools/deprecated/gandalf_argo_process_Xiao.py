...
polygon = [[-77, 33], [-100, 30], [-96, 16], [-77, 13], [-77, 33]]  # change to the real polygon
...
def get_platform_profiles(platform_number):
    ...
    try:
        ...
        logging.warning('get_platform_profiles(): %d has invalid JSON',
                        platform_number)
        return False
...
def build_argo_plots(platform):
    ...
    if platform_profiles:
        ...
        # if the temp column is all none value, remove the column and the sensor
        if slim_df['temp'].isna().all():
            argo_sensors.remove('temp')
            slim_df = slim_df.drop('temp', axis=1)

        # if the psal column is all none value, remove the column and the sensor
        if slim_df['psal'].isna().all():
            argo_sensors.remove('psal')
            slim_df = slim_df.drop('psal', axis=1)
...
def argo_process():
    ...
    logging.info('argo_process(): Argovis returned %d floats', len(platform_list))

    argo_features = []

    for platform in platform_list:
        argo_features.append(build_argo_plots(platform))
        logging.info(f'above platform number: {platform}   \n')
...
def get_bbox_floats():
    ...
    for profile in profiles:
        ...
        if platform.isnumeric():
            ...
            logging.info(f'the profile can not be added {profile} \n')
